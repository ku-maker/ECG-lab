...
- Toggle educational waveform annotations for P waves, QRS complexes, T waves, PR interval, and RR interval in supported learning views. Annotations pause the waveform while enabled so learners can compare labels against a static ECG strip.
...
- **Annotations / 波形ラベル**：ONにすると波形が一時停止し、静止した波形に対してP波、QRS、T波、PR間隔、RR間隔などを学習用の目安として確認できます。
...
selected case. Supported cases can show optional educational waveform
annotations for common landmarks such as P wave, QRS, T wave, PR interval, and
RR interval. Annotation mode pauses the waveform while enabled so learners can
compare labels against a static ECG strip.
...
width, PR interval behavior, and rhythm regularity. The same optional
educational annotations can be toggled for both compared waveforms, pausing both
strips while labels are visible.
