...
- [ ] `scripts/verify-lead-projection-all.mjs` が下記を assert、1つでも外れたら exit 1。`npm run verify:leadproj` が exit 0：
  ...
  2. **誘導ごとに波形が異なる**：全12誘導の波形ベクトルについて、任意の相異なるペアが完全一致でない（少なくとも `I` と `aVR` は Z-score 相関が負＝概ね反転）。
...
- `selectedLead` が本当に波形へ反映されているか（項目2＝誘導ごとに違う）。旧 `buildNsrPath`（nsr 固定）が残存していないか。
