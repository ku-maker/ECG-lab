...
- [ ] `scripts/verify-evaluate.mjs` が下記を assert、1つでも外れたら exit 1。`npm run verify:evaluate` が exit 0：
  ...
  3. **biphasic の符号**：`r` 時刻の Lead II 投影相当（`out.x*0.5 + out.y*(-0.8660254)`）が **強い正**（>0.5）、かつ QRS 尾部 `(r, qrsOff+40ms]` に **0 を下回る S 谷が存在**（`min < 0`）。→ R の陽性支配と、terminalS による終末陰性偏位（biphasic）を関数レベルで確認。
     - ※ 当初 spec は「`q` 時刻で負／`s` 時刻で負寄り」としていたが、実測で `q`=+0.37・`s`=+0.04（mainR の Gaussian 裾が q/s の中心時刻まで届き合計を持ち上げる）と判明したため、biphasic の本質＝「R 直後に合計が 0 を割る S 谷」で検証する形に訂正した（実測 S 谷 = −0.060 @ t=440ms）。単一の正 Gaussian では尾部が負にならないため、この谷<0 は `terminalS` を消すと必ず落ちる番人になる。
